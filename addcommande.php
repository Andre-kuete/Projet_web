...
if(!empty($_POST["inputidcl"])&&!empty($_POST["inputdate"])){
    ...
    $idcmd=$pdo->lastInsertId();//detecte le dernier ID qui a été insérer a l'interieure de la table commande
    $query2="insert into ligne_commande(idarticle,idcommande,quantite) values (:idart,:idcmd,:quant)";
    $pdostmt2=$pdo->prepare($query2);
    $pdostmt2->execute(["idart"=>$_POST["inputidarticle"],"idcmd"=>$idcmd,"quant"=>$_POST["inputqte"]]);
    $pdostmt2->closeCursor();//liberer le pdostmnt de la zone memoire
